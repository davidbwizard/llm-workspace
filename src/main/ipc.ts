...
import { createStager } from './staging.ts';
...
export function sendKeysFor(pid: unknown, raw: unknown, deps: KeysDeps = {}, images: string[] = []): KeysResult {
  if (typeof pid !== 'number' || !Number.isInteger(pid) || pid <= 0) {
    return { status: 'refused', reason: 'invalid_pid' };
  }
  // Staged image paths (src/main/staging.ts) are generated by main, but each
  // is still checked before it is wrapped in single quotes: an absolute path
  // with no quote and no control character cannot end its own quoting.
  if (images.some(p => typeof p !== 'string' || !p.startsWith('/') || /['\x00-\x1f\x7f]/.test(p))) {
  ...
  // Images may go without any text; whitespace beside them counts as none.
  const clean = images.length > 0 && (typeof raw !== 'string' || raw.trim() === '')
  ...
  // 2026-09-17). The text follows as its own paste, after a space.
  const chunks = [
    ...images.map(p => `'${p}'`),
    ...(clean.text ? [images.length ? ` ${clean.text}` : clean.text] : []),
  ];
...
export function registerIpc(
  ...
  ipcMain.handle('session:keys', (_event, pid: unknown, text: unknown, imageIds: unknown): KeysResult => {
    // Ids the stager issued, resolved to the files it wrote. An id it does
    // not know (a stale chip after an app restart, or anything forged)
    // refuses the whole send rather than silently dropping an image.
    const ids = imageIds === undefined ? [] : imageIds;
    if (!Array.isArray(ids) || ids.length > MAX_ATTACHMENTS) return { status: 'refused', reason: 'attachment_gone' };
    const paths = ids.map(id => stager.pathFor(id));
    if (paths.some(p => p === null)) return { status: 'refused', reason: 'attachment_gone' };
    return sendKeysFor(pid, text, {}, paths as string[]);
  });
