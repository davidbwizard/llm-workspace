...
export function sendKeysFor(
  pid: unknown, raw: unknown, deps: KeysDeps = {}, images: string[] = [], files: string[] = [],
): KeysResult {
  if (typeof pid !== 'number' || !Number.isInteger(pid) || pid <= 0) {
    return { status: 'refused', reason: 'invalid_pid' };
  }
  // Staged paths (src/main/staging.ts) are generated by main, but each is
  // still checked before it is wrapped in single quotes: an absolute path
  // with no quote and no control character cannot end its own quoting.
  if ([...images, ...files].some(p => typeof p !== 'string' || !p.startsWith('/') || /['\x00-\x1f\x7f]/.test(p))) {
    return { status: 'refused', reason: 'attachment_gone' };
  }
  ...
  // Attachments may go without any text; whitespace beside them counts as none.
  const clean = images.length + files.length > 0 && (typeof raw !== 'string' || raw.trim() === '')
    ? { ok: true as const, text: '' }
    : sanitizeOutbound(raw, { multiline: true });
  ...
  // Attached images go first, each as a paste of its single-quoted path and
  // nothing else: both Claude Code and Codex attach an image only when a
  // paste is the path alone, and Codex only when it is quoted (measured
  // 2026-09-17). The text follows as its own paste, after a space, led by
  // one "Attached file:" line per other file -- those the agent reads from
  // the path with its own tools rather than attaching.
  const body = [...files.map(p => `Attached file: '${p}'`), ...(clean.text ? [clean.text] : [])].join('\n');
  const chunks = [
    ...images.map(p => `'${p}'`),
    ...(body ? [images.length ? ` ${body}` : body] : []),
  ];
  let before = captured.stdout;
  for (const [i, chunk] of chunks.entries()) {
    const buffer = nextPasteBuffer();
    const loaded = loadBuffer(name, buffer, chunk, deps.send);
    if (!loaded.ok) {
      // The user is shown a generic "That session has ended."; the actual
      // tmux error would otherwise be discarded entirely. With images, an
      // earlier chunk may already sit in the input line; no Enter follows,
      // so nothing half-delivered is ever submitted.
    ...
    // it. Give it a bounded chance to visibly react before going on.
    ...
    if (!waitForPasteToSettle(name, before, deps.capture, deps.sleep ?? defaultSleep)) {
      console.error('tmux paste settle timed out, sending Enter anyway:', { pid, name });
    }
    // The next paste settles against what the pane shows now.
    if (i < chunks.length - 1) {
      const now = capturePane(name, PASTE_SETTLE_CAPTURE_LINES, deps.capture);
...
export function registerIpc(
  ...
  // Images attached in the conversation pane wait here, as files main
  // wrote, until they are sent (src/main/staging.ts). The OS temp folder is
  // per-user on macOS; the folder itself is created 0700.
  const stager = createStager(join(tmpdir(), 'llm-workspace-attachments'));
  void stager.sweep();
  // Other files, kept a week under their own (cleaned) names.
  const fileStager = createFileStager(join(tmpdir(), 'llm-workspace-files'));
  void fileStager.sweep();
  ...
  ipcMain.handle('session:keys', (_event, pid: unknown, text: unknown, attach: unknown): KeysResult => {
    ...
    // refuses the whole send rather than silently dropping an attachment.
    const gone: KeysResult = { status: 'refused', reason: 'attachment_gone' };
    const a = (attach ?? {}) as { images?: unknown; files?: unknown };
    if (typeof a !== 'object') return gone;
    const imageIds = a.images ?? [];
    const fileIds = a.files ?? [];
    if (!Array.isArray(imageIds) || !Array.isArray(fileIds)
      || imageIds.length + fileIds.length > MAX_ATTACHMENTS) return gone;
